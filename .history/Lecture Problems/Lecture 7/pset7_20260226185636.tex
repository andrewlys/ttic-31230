...
\[
\Phi(t + \Delta t) = \Phi(t) - g\Delta t + \epsilon \sqrt{\Delta t}, \quad \epsilon \cN(0, \eta \Sigma^B)
\]
Show that for $\eta = B \eta_0$ the stationary distribution is determined by $\eta_0$ independent of $B$. 
\begin{proof}
    We have:
    \[
    \frac{1}{2}\eta \Sigma(\Phi)^{-1}(\nabla_\Phi \log(p)) = - \Sigma(\Phi)^{-1} \nabla_\Phi \cL
    \]
    Plugging in $\eta = B \eta_0$ and $\Sigma(\Phi) = \Sigma^B$ we have:
    \[
    \frac{1}{2}B \eta_0 (\Sigma^B)^{-1}(\nabla_\Phi \log(p)) = - \Sigma^B(\Phi)^{-1} \nabla_\Phi \cL
    \]
\end{proof}
\end{document}