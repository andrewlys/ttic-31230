...
\subsection*{Part (a)} Give an equation computing tensor $\tilde{W}^Q[K, J, J]$ from $W^Q$ and $W^K$ such that the attention can be written as:
\[
\alpha_{\ell + 1} (k, t_1, t_2) = \underset{t_2}{\mathrm{softmax}}\, \left[
    L_{\ell} [t_1, J_1] \tilde{W}^Q [k, J_1, J_2] L_{\ell} [t_2, J_2]
\right]
\]
\begin{proof}
    We want:
    \[
    \tilde{W}^Q[k, j_1, j_2] = \frac1{\sqrt{I}}W^Q[k, j_1, I] W^K[k, I, j_2]
    \]
    Thus, we have 
    \[
    \tilde{W}^Q[k, J_1, J_2] = \frac1{\sqrt{I}}W^Q[k, J_1, I] W^K[k, I, J_2]
    \]
\end{proof}
\subsection*{Part (b)} What is the resulting number of query matrix parameters for a given layer, 
and how does this compare to the number of key-query matrix parameters for a given layer in the original transformer version.
\begin{proof}
    We have $K$ heads, and each head has $J^2$ parameters, so we have $KJ^2$ parameters.
    In the original, we have $K$ heads, and each of the query and key matrices have $IJ$ parameters, so we have $2KIJ$ parameters.
    If $J < 2I$, then we have fewer parameters than the original version, but if $I < \frac{J}2$, then we have more parameters than the original version.
\end{proof}
\end{document}
