...
\subsection*{Problem 5. RNN run time} Consider an autoregressive RNN neural language model with 
\[
\Pr_\Phi (w_t | w_1, \ldots, w_{t - 1}) = \underset{w'}{\mathrm{softmax}} e[w', I] h[t - 1, I]
\]
$e[w, I]$ is the word vector for word $w$ and $h[t, I]$ is the hidden state vector at time $t$ of a left-to-right RNN.
...
\begin{align*}
  \Phi^\ast &= \arg\min_\Phi \underset{w_1, \ldots, w_T \sim \mathrm{Train}}{\ee} - \log \Pr_\Phi(w_1, \ldots, w_T) \\
  &= \arg\min_\Phi \underset{w_1, \ldots, w_T \sim \mathrm{Train}}{\ee} \sum_{t = 1}^T - \log \Pr_\Phi(w_t | w_1, \ldots, w_{t - 1})
\end{align*}

What is the order of run time as a function of sentence length $T$ for the backpropagation for this model run on a sentence $w_1, \ldots, w_T$?
\begin{proof}
  It is $O(T)$, since the length of the assignment program is of the order $O(T)$. 
\end{proof}
\end{document}
